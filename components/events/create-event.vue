...
const submit = async () => {
  ...
  const { errors } = await client.models.Events.create({
    ...
    type: data.type,
    cost: JSON.stringify([data.cost,cost_interview]),
  });
  console.log(errors);
  if (!errors) {
      ...
      <div class="py-8">
        ...
        >
          <template v-slot:item.1>
            <v-form @submit.prevent="nextStep" class="pt-4 flex flex-col gap-4">
              ...
              <div>
                ...
                <v-textarea
                  v-model="data.description"
                  label="Recomendacsión Retiro y/o Ceremonia"
                  variant="outlined"
                ></v-textarea>
                <v-textarea
          ...
          <template v-slot:item.3>
            <div class="flex gap-4 mt-8 border border-gray-200 rounded-lg px-4 py-4">
              <v-text-field
